...
local gunSoundId = "rbxassetid://12222242"
local killSoundEnabled = false
local killSoundId = "rbxassetid://6026984224"
...
local function isCombatVisual(object)
    if not (object:IsA("Beam") or object:IsA("Trail") or object:IsA("ParticleEmitter")) then
        return false
    end
    local name = cleanName(object.Name)
...
local function bindDeathSound(player)
    local function bindCharacter(character)
        local humanoid = character:FindFirstChildOfClass("Humanoid") or character:WaitForChild("Humanoid", 5)
        if humanoid then
            local connection = humanoid.Died:Connect(function()
                if killSoundEnabled then playEffectSound(killSoundId) end
            end)
...
for _, player in ipairs(Players:GetPlayers()) do
    bindDeathSound(player)
end
local deathPlayerConnection = Players.PlayerAdded:Connect(bindDeathSound)
...
effectsTab:CreateToggle({ name = "Gun sound override", value = false, callback = function(value)
    ...
    if value then
        for _, object in ipairs(Workspace:GetDescendants()) do
            if isGunSound(object) then
                pcall(function() object.SoundId = gunSoundId end)
            end
        end
    else
        notify("Effects", "Gun sound override disabled.", 2, 91452555903853)
    end
end })
effectsTab:CreateInput({ name = "Gun sound ID", value = gunSoundId, placeholder = "rbxassetid://...", callback = function(value)
...
effectsTab:CreateToggle({ name = "Kill sound", value = false, callback = function(value)
    killSoundEnabled = value
end })
effectsTab:CreateInput({ name = "Kill sound ID", value = killSoundId, placeholder = "rbxassetid://...", callback = function(value)
